...
public class ColorSimplifier {
    ...
    public static int getColorsCount(BufferedImage image){
    ...
    public static BufferedImage colorize(BufferedImage image,int maxColors) {
        ConcurrentMap<Color, Double> colorDifferenceMap = new ConcurrentHashMap<>();
        ExecutorService executorService = Executors.newFixedThreadPool(
            Runtime.getRuntime().availableProcessors());
        generateInfiniteColors().distinct().limit(getColorsCount(image)).forEach(color -> {
        ...
        for (int y = 0; y < image.getHeight(); y++) {
            for (int x = 0; x < image.getWidth(); x++) {
                CIELab cieLabColor = CIELab.from(new Color(image.getRGB(x, y)));
                ...
                colorSolutions.stream()
                    ...
                    .ifPresent(closestColor ->
                            image.setRGB(finalX, finalY, closestColor.getRGB()));
            }
        }
